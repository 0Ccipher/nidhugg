...
#include <config.h>
#ifndef __CM_TRACE_BUILDER_H__
#define __CM_TRACE_BUILDER_H__
...
class CMTraceBuilder final : public TSOPSOTraceBuilder{
public:
  CMTraceBuilder(const Configuration &conf = Configuration::default_conf);
  virtual ~CMTraceBuilder() override;
  ...
  (const SymData &sd, const SymData::block_type expected, bool suCMess) override;
...
protected:
  ...
  /* An ACMess is a pair (tp,ml) representing an aCMess to
   * memory. ACMesses come in two varieties:
   *
   * (W_ALL_MEMORY,0) is considered as a store aCMess to all memory.
   *
   * (tp,ml) with tp in {R,W}, is a Read or Write aCMess to the byte
   * indicated by the pointer ml.
   */
  class ACMess{
  public:
    /* The type of memory aCMess. */
    enum Type {R, W, W_ALL_MEMORY, NA} type;
    /* The aCMessed byte. */
    const void *ml;
    bool operator<(const ACMess &a) const{
    ...
    ACMess() : type(NA), ml(0) {};
    ACMess(Type t, const void *m) : type(t), ml(m) {};
  };
  ...
  public:
    ...
    int delivery_indices; //transaction delivery
  ...
  public:
    ...
    /* An index into prefix, to the latest update that aCMessed this
  ...
  class Mutex{
  public:
    Mutex() : last_aCMess(-1), last_lock(-1), locked(false) {};
    Mutex(int laCM) : last_aCMess(laCM), last_lock(-1), locked(false) {};
    int last_aCMess;
    int last_lock;
    bool locked;
  };
  ...
  public: 
    ...
    std::vector<std::pair<IPid , Tid>> modification_order;
  ...
  public:
     ...
     * nondeterministically. (E.g. malloc may suCMeed or fail
  ...
  CMSchedules CMschedules;
  std::unordered_map<int,std::shared_ptr<CMSEvent>> current_schedules; // depth->SEvent
  ...
  bool is_equivalent(std::vector<std::vector<CMSTransaction>> &schedules , std::vector<CMSTransaction> this_schedule);
  void add_to_queue();
  void append_replay_events(std::vector<CMSTransaction> &transactions_before , std::vector<Transaction> &new_transactions, 
