...
public:
  CCTraceBuilder(const Configuration &conf = Configuration::default_conf);
  virtual ~CCTraceBuilder() override;
...
protected:
  ...
  CCSchedules ccschedules;
  std::unordered_map<int,std::shared_ptr<CCSEvent>> current_schedules; // depth->SEvent
  ...
  bool is_equivalent(std::vector<std::vector<CCSTransaction>> &schedules , std::vector<CCSTransaction> this_schedule);
  void add_to_queue();
  void append_replay_events(std::vector<CCSTransaction> &transactions_before , std::vector<Transaction> &new_transactions, 
