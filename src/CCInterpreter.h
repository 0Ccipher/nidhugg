...
#include <config.h>
#ifndef __CC_INTERPRETER_H__
#define __CC_INTERPRETER_H__

#include "Interpreter.h"
#include "CCTraceBuilder.h"

/* A CCInterpreter is an interpreter running under the CC
 * semantics. The execution should be guided by scheduling from a
 * CCTraceBuilder.
 */
class CCInterpreter : public llvm::Interpreter{
public:
  explicit CCInterpreter(llvm::Module *M, CCTraceBuilder &TB,
                          const Configuration &conf = Configuration::default_conf);
  virtual ~CCInterpreter();

  static std::unique_ptr<CCInterpreter>
  create(llvm::Module *M, CCTraceBuilder &TB,
         const Configuration &conf = Configuration::default_conf,
...
protected:
  ...
  class CCThread{
  public:
    CCThread() : partial_buffer_flush(-1){};
    /* The CC store buffer of this thread. Newer entries are further
     * to the back.
     */
   ...
   * created.
   */
  std::vector<CCThread> CC_threads;
};
