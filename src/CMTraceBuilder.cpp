...
CMTraceBuilder::CMTraceBuilder(const Configuration &conf)
...
CMTraceBuilder::~CMTraceBuilder(){
}

bool CMTraceBuilder::schedule(int *proc, int *aux, int *alt, bool *dryrun){
  if (cancelled) {
  ...
  *dryrun = false;
  *alt = 0;
  *aux = -1; /* No auxilliary threads in CM */
  if(replay){
...
void CMTraceBuilder::refuse_schedule(){
...
void CMTraceBuilder::mark_available(int proc, int aux){
  threads[ipid(proc,aux)].available = true;
}

void CMTraceBuilder::mark_unavailable(int proc, int aux){
  threads[ipid(proc,aux)].available = false;
}

bool CMTraceBuilder::is_replaying() const {
  return prefix_idx < replay_point;
}

void CMTraceBuilder::cancel_replay(){
  assert(replay == is_replaying());
  cancelled = true;
  replay = false;
}

void CMTraceBuilder::metadata(const llvm::MDNode *md){
  if(curev().md == 0){
    curev().md = md;
  }
  last_md = md;
...
bool CMTraceBuilder::sleepset_is_empty() const{
  return true;
}

Trace *CMTraceBuilder::get_trace() const{
...
void CMTraceBuilder::append_replay_events(std::vector<CMSTransaction> &transactions_before , std::vector<Transaction> &new_transactions, 
                                        std::vector<CMSEvent> &events_before, std::vector<Event> &new_prefix){
  //earlier transactions for replay
  for(int i= 0 ; i < transactions_before.size() ; i++){
    CMSTransaction &st =  transactions_before[i];
...
bool CMTraceBuilder::reset(){
  ...
  //temp = CMschedules.scheduler.top()->depth;
  int cur_events = -1;
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////
  while(true){
    if(!CMschedules.scheduler.top()->new_read_from.empty() || !CMschedules.scheduler.top()->new_schedules->scheduled_transactions.empty()){
      break;
    }
    else{
      current_schedules.erase(CMschedules.scheduler.top()->depth);
      CMschedules.scheduler.pop();
    }
  }
  if(!CMschedules.scheduler.top()->new_read_from.empty() || !CMschedules.scheduler.top()->new_schedules->scheduled_transactions.empty()) {
    if(!CMschedules.scheduler.top()->new_read_from.empty()) { //New read source
      cur_events = CMschedules.scheduler.top()->depth;
      cur_transactions = CMschedules.scheduler.top()->tid - 1;

      int new_read = CMschedules.scheduler.top()->new_read_from.back();
      CMschedules.scheduler.top()->read_from = new_read;
      CMschedules.scheduler.top()->new_read_from.pop_back();//remove this read
      CMschedules.scheduler.top()->new_schedules->new_read_from.pop_back();

      //add earlier events for replay
      append_replay_events(CMschedules.scheduler.top()->new_schedules->transactions_before, new_transactions, 
                                      CMschedules.scheduler.top()->new_schedules->events_before, new_prefix);
      //add current event to replay
      {
        int pid = CMschedules.scheduler.top()->iid.get_pid();
        int index = CMschedules.scheduler.top()->iid.get_index();
        IID<IPid> eiid(pid, index);
        new_prefix.emplace_back(eiid);
        new_prefix.back().size = CMschedules.scheduler.top()->size;
        new_prefix.back().sym = CMschedules.scheduler.top()->sym;
        new_prefix.back().pinned = CMschedules.scheduler.top()->pinned;
        new_prefix.back().read_from = new_read;
        new_prefix.back().localread = CMschedules.scheduler.top()->localread;
        new_prefix.back().swappable = CMschedules.scheduler.top()->swappable;
        new_prefix.back().current = CMschedules.scheduler.top()->current;
        new_prefix.back().depth = CMschedules.scheduler.top()->depth;
        new_prefix.back().tid = CMschedules.scheduler.top()->tid; 
        new_prefix.back().new_schedules = CMschedules.scheduler.top()->new_schedules;
      }
    }
    //Schedules
    else{
      ...
      std::vector<CMSTransaction> &schedule = CMschedules.scheduler.top()->new_schedules->scheduled_transactions.back();
      for(unsigned i=0 ; i < schedule.size() ; i++) {
        CMSTransaction &st = schedule[i];
        Transaction tr(st.get_pid() , st.get_tid() , st.get_index());
        new_transactions.emplace_back(tr);
      }
      //update read source
      CMschedules.scheduler.top()->read_from = new_transactions.back().tid - 1;
      ...
      for(int j = new_prefix.size()-1 ; j >= 0 ; j--) {
        if(new_prefix[j].tid == CMschedules.scheduler.top()->new_schedules->transactions_before.back().tid - 1 ) {
          break;
        }
        if(new_prefix[j].tid == CMschedules.scheduler.top()->new_schedules->transactions_before.back().tid) {
      ...
      }
      {
        ...
        std::vector<CMSEvent> &scheduled_events = CMschedules.scheduler.top()->new_schedules->scheduled_events.back();
        ...
        CMschedules.scheduler.top()->new_schedules->scheduled_events.pop_back();
        CMschedules.scheduler.top()->new_schedules->scheduled_transactions.pop_back();
        ...
        new_prefix.back().pinned = CMschedules.scheduler.top()->pinned;
        new_prefix.back().read_from = current_tid - 2;//CMschedules.scheduler.top()->read_from;//updated read
        new_prefix.back().localread = CMschedules.scheduler.top()->localread;
        new_prefix.back().swappable = false;
        new_prefix.back().current = CMschedules.scheduler.top()->current;
...
IID<CPid> CMTraceBuilder::get_iid() const{
  IPid pid = curev().iid.get_pid();
...
std::string CMTraceBuilder::iid_string(std::size_t pos) const{
  return iid_string(prefix[pos]);
}

std::string CMTraceBuilder::iid_string(const Event &event) const{
...
std::string CMTraceBuilder::iid_string(IID<IPid> iid) const{
  return iid_string(find_process_event(iid.get_pid(), iid.get_index()));
}

void CMTraceBuilder::debug_print() const {
  llvm::dbgs() << "CMTraceBuilder (debug print, replay until " << replay_point << "):\n";
  int idx_offs = 0;
  int iid_offs = 0;
  int rf_offs = 0;
  int clock_offs = 0;
...
bool CMTraceBuilder::store(const SymData &sd){
  assert(false && "Cannot happen");
  abort();
  return true;
}

bool CMTraceBuilder::atomic_store(const SymData &sd){
...
void CMTraceBuilder::do_atomic_store(const SymData &sd){
...
bool CMTraceBuilder::atomic_rmw(const SymData &sd){
...
bool CMTraceBuilder::load(const SymAddrSize &ml){
...
void CMTraceBuilder::do_load(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool CMTraceBuilder::compare_exchange
(const SymData &sd, const SymData::block_type expected, bool suCMess){
  if(suCMess){
...
bool CMTraceBuilder::full_memory_conflict(){
  invalid_input_error("CM does not support black-box functions with memory effects");
  return false;
  if (!record_symbolic(SymEv::Fullmem())) return false;
  curev().may_conflict = true;

  // /* See all pervious memory aCMesses */
  // for(auto it = mem.begin(); it != mem.end(); ++it){
  //   do_load(it->second);
  // }
  // last_full_memory_conflict = prefix_idx;

  // /* No later aCMess can have a conflict with any earlier aCMess */
  // mem.clear();
  return true;
}

bool CMTraceBuilder::fence(){
  return true;
}

bool CMTraceBuilder::join(int tgt_proc){
...
void CMTraceBuilder::add_happens_after(unsigned second, unsigned first){
  assert(first != ~0u);
  assert(second != ~0u);
  assert(first != second);
  assert(first < second);
...
void CMTraceBuilder::add_happens_after_thread(unsigned second, IPid thread){
...
bool CMTraceBuilder::record_symbolic(SymEv event){
  if (!replay) {
    assert(!seen_effect);
    /* New event */
...
bool CMTraceBuilder::is_load(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  return e.kind == SymEv::LOAD_CMV;
}

bool CMTraceBuilder::is_minit(unsigned i) const {
  return prefix[i].sym.kind == SymEv::M_INIT;
}

bool CMTraceBuilder::is_mdelete(unsigned i) const {
  return prefix[i].sym.kind == SymEv::M_DELETE;
}


bool CMTraceBuilder::is_store(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  return e.kind == SymEv::STORE_CMV;
}


bool CMTraceBuilder::is_store_when_reading_from(unsigned i, int read_from) const {
  const SymEv &e = prefix[i].sym;
...
SymAddrSize CMTraceBuilder::get_addr(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  if (e.has_addr()) {
    return e.addr();
  }
...
SymData CMTraceBuilder::get_data(int i, const SymAddrSize &addr) const {
  if (i == -1) {
...
bool CMTraceBuilder::happens_before(const Event &e,
...
std::vector<int> CMTraceBuilder::iid_map_at(int event) const{
...
void CMTraceBuilder::iid_map_step(std::vector<int> &iid_map, const Event &event) const{
  if (iid_map.size() <= unsigned(event.iid.get_pid())) iid_map.resize(event.iid.get_pid()+1, 1);
  iid_map[event.iid.get_pid()] += event.size;
}

void CMTraceBuilder::iid_map_step_rev(std::vector<int> &iid_map, const Event &event) const{
  iid_map[event.iid.get_pid()] -= event.size;
}

inline Option<unsigned> CMTraceBuilder::try_find_process_event(IPid pid, int index) const{
...
inline unsigned CMTraceBuilder::find_process_event(IPid pid, int index) const{
  assert(pid >= 0 && pid < int(threads.size()));
...
long double CMTraceBuilder::estimate_trace_count() const{
  return estimate_trace_count(0);
}

bool CMTraceBuilder::check_for_cycles() {
  return false;
}

long double CMTraceBuilder::estimate_trace_count(int idx) const{
...
int CMTraceBuilder::compute_above_clock(unsigned i) {
  int last = -1;
  ...
  transactions[i].above_clock[ipid] = std::max(tidx,threads[ipid].delivery_indices);
  
  return last;
}

void CMTraceBuilder::compute_vclocks(){
  ...
  for (int i = 0; i <= transaction_idx; i++){
    ...
    if(!transactions[i].modification_order.empty()){
      for(int j = 0 ; j < transactions[i].modification_order.size() ; ++j){
        if(transactions[i].modification_order[j].second != -1){
          transactions[i].clock = transactions[i].clock.expand(transactions[i].modification_order[j].first + 1);
          transactions[i].clock[transactions[i].modification_order[j].first] = transactions[i].modification_order[j].second; //clock[ipid] = mo
        }
      }
    } // [pco]
  }
...
inline unsigned CMTraceBuilder::find_process_transaction(IPid pid, int index) const{
  assert(pid >= 0 && pid < int(threads.size()));
...
void CMTraceBuilder::add_transaction_happens_after(Tid second, Tid first){
  assert(first != ~0u);
  assert(second != ~0u);
  assert(first != second);
  assert(first < second);
...
void CMTraceBuilder::add_transaction_happens_after_thread(Tid second, IPid thread){
...
bool CMTraceBuilder::transaction_happens_before(const Transaction &t,
                                     const VClock<int> &clock) const {
  IID<IPid> tiid(t.get_pid(),t.get_index());
  return clock.includes(tiid);
}

bool CMTraceBuilder::has_store_on_var(const void *ptr, unsigned i) const {
...
void CMTraceBuilder::createNextEvent(){
  //Are we replaying
  if(replay){
    if (0 <= prefix_idx && threads[curev().iid.get_pid()].last_event_index() <
        curev().iid.get_index() + curev().size - 1) {
...
bool CMTraceBuilder::is_begin(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  return e.kind == SymEv::BEGIN;
}

bool CMTraceBuilder::is_end(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  return e.kind == SymEv::END;
}

void CMTraceBuilder::beginTransaction(int tid) {
  if(replay){
    assert(tid > 0);
    IPid pid = curev().iid.get_pid();
...
void CMTraceBuilder::endTransaction(int tid) {
  if(replay){
    curev().tid = tid;
...
int CMTraceBuilder:: performWrite(void *ptr, llvm::GenericValue val , int typ){
  if(!typ)
    return -1;
  bool r = record_symbolic(SymEv::CMVStore(transactions[transaction_idx].tid));
...
void CMTraceBuilder::record_replay(int eindex , int tindex){

    std::vector<CMSEvent> &events = prefix[eindex].new_schedules->events_before;
    ...
    std::vector<CMSTransaction> &trans = prefix[eindex].new_schedules->transactions_before;
...
int CMTraceBuilder::performRead(void *ptr , int typ) {
  if(!typ)
    return -1;

  bool r = record_symbolic(SymEv::CMVLoad(transactions[transaction_idx].tid));
  if(!transactions.empty() && curev().iid.get_pid() == transactions[transaction_idx].pid) 
    curev().tid = transactions[transaction_idx].tid;
  curev().may_conflict = true;
  ...
  for (int j = 0; j < transaction_idx ; ++j) {
    if (has_store_on_var(ptr,j)){
      ...
      if(transactions[j].clock[cur_transaction.get_pid()] <= cur_transaction.clock[cur_transaction.get_pid()]){
        delivered_before.insert(j);
      } ////////////////////////
    }
  }
  ...
  for(auto j : happens_before){
    int readable = true;
    for( auto k : happens_before){ // hb
      if(j!= k && transaction_happens_before(transactions[j] , transactions[k].above_clock)){
        readable = false;
      }
    }
    //pco[] this process---if this tran [pco] delivered_before[k] then not readable /////////////////////////////////////////
    for(auto k : delivered_before){
      if(j != k && transactions[j].clock[cur_transaction.get_pid()] < transactions[k].clock[cur_transaction.get_pid()]){ //TODO:Ensure t[k].clock[pid] != 0
        readable = false;
      }
    }
    if(readable){
  ...
  transactions[transaction_idx].hb_writes[ptr] = latest_hb_writes;////
    for(auto j: writes){
      ...
      for(auto k : delivered_before){
        if(j != k && transactions[j].clock[cur_transaction.get_pid()] < transactions[k].clock[cur_transaction.get_pid()]){
          readable = false;
          break;
        }// t[j] [pco] t[k]
        if(j != k && add_transaction_happens_after_thread(transactions[j],transactions[k].above_clock)){
          readable = false;
          break;
        } //t[j] [hb] t[k] 
      }
      ...
      for(auto tr : cur_reads){
        ...
        if(tr.second != -1 && transactions[j].global_variables.count(tr.first) && tr.second != j 
                           && transactions[tr.second].global_variables.count(ptr) ){
          readable = false;
          break;
        } //cond2
        ...
        if(tr.second != -1 && transactions[j].global_variables.count(tr.first) && tr.second != j 
                          && transaction_happens_before(transactions[tr.second] , transactions[j].above_clock)){
          readable = false;
          break; 
        }  // [po U rf]
        if(tr.second != -1 && transactions[j].global_variables.count(tr.first) && tr.second != j 
                          && transactions[tr.second].clock[cur_transaction.get_pid()] < transactions[j].clock[cur_transaction.get_pid()]){
          readable = false;
          break; 
        } // [pco]
        ...
        if(readable){
          for(auto tr1: latest_hb_writes){
            if(transactions[j].global_variables.count(tr.first) && transactions[tr1].global_variables.count(ptr) && tr1!= j
                        && transaction_happens_before(transactions[tr.second] , transactions[tr1].above_clock)){
              readable = false;
              break;
            } //[po U rf]
            if(transactions[j].global_variables.count(tr.first) && transactions[tr1].global_variables.count(ptr) && tr1!= j
                        && transactions[tr.second].clock[cur_transaction.get_pid()] < transactions[tr1].clock[cur_transaction.get_pid()]){
              readable = false;
              break;
            } //[pco]
          }
        ...
        if(readable){
          for( int k=0; k < transaction_idx ; k++){
            if(tr.second == -1 && transaction_happens_before(transactions[k] , transactions[j].above_clock) 
                        && transactions[k].global_variables.count(tr.first)){
              readable = false;
              break;
            }
            ...
            if(tr.second != -1 && transaction_happens_before(transactions[k] , transactions[j].above_clock) 
                        && transactions[k].global_variables.count(tr.first) && k != tr.second
                        && transaction_happens_before(transactions[tr.second] , transactions[k].above_clock)){
              readable = false;
              break;
            }
          }
...
void CMTraceBuilder::add_to_queue(){
  ...
  //Add event to queue
  CMschedules.scheduler.push(sevent);
  current_schedules.insert({sevent->depth , sevent}); //Added to umap for fast look up
}

void CMTraceBuilder::createSchedule(int tid){
  IPid pid = curev().iid.get_pid();
  Transaction &t = transactions[transaction_idx];
      ...
      for(int i = transaction_idx-1 ; i >= 0 ; i--){
        if(transactions[i].current_reads.count(ptr.first)){
          if(!transaction_happens_before(transactions[i],t.above_clock)) {
            ...
            //*** tr[i] [hb] tr[j] AND tr[j] [hb] t....This violates CM as cycle is formed
            if(possible){
            ...
            if(possible){
              ...
              std::vector<CMSTransaction> trns_schedule;
              ...
              for( int j = i+1 ; j <= transaction_idx; j++){
                if(transaction_happens_before(transactions[j],t.above_clock)){
                  CMSTransaction tr(transactions[j].get_pid(), ++cur_tid , transactions[j].get_index());
              ...
              for( ; ; j--){
                if(prefix[j].tid == transactions[i].get_tid()){
                  ...
                  //////////////////////////////////////////
                  {
                    ...
                    if(sym.kind == SymEv::BEGIN || sym.kind == SymEv::END || sym.kind == SymEv::LOAD_CMV || sym.kind == SymEv::STORE_CMV)
...
bool CMTraceBuilder::is_equivalent(std::vector<std::vector<CMSTransaction>> &schedules , std::vector<CMSTransaction> this_schedule) {
  if(schedules.empty())
    return false;
  bool res = false;
  for(int i=0 ; i < schedules.size() ; i++){
    std::vector<CMSTransaction> &schedule = schedules[i];
    int count = 0;
...
uint64_t CMTraceBuilder::tracecount(){
  uint64_t t = 0;
  int64_t value = 0;
  for(unsigned i=0 ; i < prefix.size() ; ++i){
